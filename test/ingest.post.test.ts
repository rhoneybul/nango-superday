...
describe('POST /ingest', () => {
  ...
  it('uses the supplied timestamp', async () => {
    const res = await request(app).post('/ingest').send({ account_id: 'acc_1', event_name: 'connection_created', timestamp: '2026-09-01T10:00:00Z' });
    expect(res.status).toBe(202);
  ...
  it.each([
    ...
    [{ account_id: '  ', event_name: 'connection_created' }, /account_id: Too small/],
    [{ account_id: 123, event_name: 'connection_created' }, /account_id: .*expected string/],
    [{ account_id: 'acc_1', event_name: 'connection_created', timestamp: 'not-a-date' }, /timestamp: must be an ISO-8601 date/],
    ...
    [{ account_id: 'acc_1', event_name: 'connection_created', timestamp: '2999-01-01T00:00:00Z' }, /timestamp: must not be in the future/],
