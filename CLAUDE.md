...
`events`: `id BIGSERIAL`, `account_id TEXT`, `event_name TEXT`, `quantity INT DEFAULT 1` (units of usage; billing sums it), `metadata JSONB NULL` (free-form, ≤ 4 KB as JSON, validated as an object), `timestamp TIMESTAMPTZ DEFAULT now()`, `created_at TIMESTAMPTZ`.
