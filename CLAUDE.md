...
src/consumer.ts  `handleMessage`: decode fail → nack requeue=false (DLQ now); duplicate (`createEvent` returns null on the `P2002` unique violation) → ack; insert fail → nack requeue=true (broker dead-letters after the delivery limit); else ack. `main()` connects (recovery), prefetch 50, consumes.
...
`events`: `id BIGSERIAL`, `account_id TEXT`, `event_id TEXT` (idempotency key, client-supplied or API-generated UUID; `UNIQUE (account_id, event_id)`), `event_name TEXT`, `metadata JSONB` (required; per-event required fields from `src/models/event-catalog.ts`, extra keys kept, ≤ 4 KB as JSON), `timestamp TIMESTAMPTZ DEFAULT now()`, `created_at TIMESTAMPTZ`.
...
- `GET /event-types` → the catalogue (`describeEventTypes()`): name, description, billable `unit`, required `metadata` fields and types.
- `POST /ingest` body `{ account_id, event_id?, event_name, metadata, timestamp? }` (`event_id` ≤ 128 chars; omitted → `randomUUID()` in `ingestEvent`) (`metadata` checked against `EVENT_CATALOG[event_name].metadata`, issues at `metadata.<field>`) → 202 `{ data: { accountId, eventName, timestamp } }` once the broker confirmed the publish (no DB id yet; the consumer inserts later). `account_id` must be a seeded account → otherwise `404 { error: "Account <id> not found" }` (checked after validation, before the rate limit, so unknown accounts consume no quota). Missing timestamp → the API's receive time. Broker down → 500.
- `POST /ingest/batch` body `{ events: [{ account_id, event_name, metadata, timestamp }] }` (1–100 events, `timestamp` required) → `{ data: { success, failed, errors: [{ index, path, message, reason }] } }`, status 202 if `success > 0`, else 429 if every error is `rate_limited`, else 400. **Partial success**: the chain passes a `BatchState { accepted, errors }` through `validateIngestBatch` (per-event zod, `reason: invalid`), `requireBatchAccounts` (`unknown_account`), `batchRateLimit` (first `INGEST_BATCH_RATE_LIMIT_PER_MINUTE`, 10 per account per minute, keys `batch:<account>`, over-limit account's events → `rate_limited`, recorded in `ingest_batch_rate_limited_*`; then every remaining event is charged one hit on the shared `<account>:<event>` counter with `INGEST_RATE_LIMIT_PER_MINUTE`, over-limit events → `rate_limited`, recorded in `ingest_rate_limited_*`, i.e. the same limit and alert as single ingest); `ingestBatch` publishes what is left. Envelope errors (no `events`, >100) are still a plain 400.
