...
`events`: `id BIGSERIAL`, `account_id TEXT`, `event_name TEXT`, `metadata JSONB NULL` (free-form, ≤ 4 KB as JSON, validated as an object), `timestamp TIMESTAMPTZ DEFAULT now()`, `created_at TIMESTAMPTZ`.
...
- `POST /ingest` body `{ account_id, event_name, metadata?, timestamp? }` → 202 `{ data: { accountId, eventName, timestamp } }` once the broker confirmed the publish (no DB id yet; the consumer inserts later). `account_id` must be a seeded account → otherwise `404 { error: "Account <id> not found" }` (checked after validation, before the rate limit, so unknown accounts consume no quota). Missing timestamp → the API's receive time. Broker down → 500.
- `POST /ingest/batch` body `{ events: [{ account_id, event_name, metadata?, timestamp }] }` (1–100 events, `timestamp` required) → `{ data: { queued, failed, errors: [{ index, path, message, reason }] } }`, status 202 if `queued > 0`, else 429 if every error is `rate_limited`, else 400. **Partial success**: the chain passes a `BatchState { accepted, errors }` through `validateIngestBatch` (per-event zod, `reason: invalid`), `requireBatchAccounts` (`unknown_account`), `batchRateLimit` (`INGEST_BATCH_RATE_LIMIT_PER_MINUTE`, 10 per account per minute, keys `batch:<account>`; an over-limit account's events → `rate_limited`, recorded in `ingest_batch_rate_limited_*`); `ingestBatch` publishes what is left. Envelope errors (no `events`, >100) are still a plain 400.
